# -*- coding: utf-8 -*-
"""
Created on Wed May 31 08:33:24 2023
...
df=pd.read_csv('C:/Gaurav/Diabetes.csv')
df
...
# Delete columns
df2=df1.drop([' Age (years)',' Body mass index'],axis=1)
print(df2)
...
##ignore error
df2=df.astype({' Class variable':int},errors='ignore')
df2.dtypes
##Generates error
df2=df.astype({' Class variable':int},errors='raise')
df2.dtypes
...
df1=df.drop(labels=[' Number of times pregnant'],axis=1)
df1
#alternatively you can also use column instead of labels
df2=df.drop(columns=[' Number of times pregnant'],axis=1)
df2
...
df2=df[' Number of times pregnant']
df2
...
#Rename column name
df2=df.rename(columns={' Number of times pregnant':'NTP'})
print(df2)
#Alternatively you can also write above code by using axis
df2=df.rename({' Number of times pregnant':'NTP',' Age (years)':'AGE'},axis=1)
print(df2)
#
df2=df.rename({' Number of times pregnant':'NTP'},axis='columns')
print(df2)
#In order to change existing dataframe without copying to existing dataframe
df.rename({' Age (years)':'AGE'},axis='columns',inplace=True)
print(df)
